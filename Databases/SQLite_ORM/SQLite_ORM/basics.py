...
class TableManager:
    ...
    def create_table(self, table_name, autoincrement_id=True, table_cols=None, add_timestamp=False):
        ...
        timestamp_col = 'timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,' if add_timestamp else ''
...
class CRUDManager:
    ...
    def upsert_by_condition(self, table_name, conditions, updates): # not tested
        existing_record = self.retrieve_by_conditions(table_name, conditions=conditions)

        if existing_record: 
            self.update_by_condition(table_name, updates, conditions)
        else:
            # Combine conditions and updates for the insert
            combined_data = {**conditions, **updates}
            columns = ", ".join(combined_data.keys())
            placeholders = ", ".join("?" for _ in combined_data)
